...
export async function main(ns) {
  ...
  const factions = [
	...
	const handleSleeve = function (sleeveIndex) {
		...
		for (const companyName of companies) {
			if (ns.getPlayer().factions.includes(companyName)) {
				...
				if (companyFavor < favorGoal) {
					if (task?.factionName !== companyName) {
  					const workPrio = ['security', 'field', 'hacking'];
						const possibleWork = new Set(ns.singularity.getFactionWorkTypes(companyName));
						if (!possibleWork.size) continue;
						try {
							ns.sleeve.setToFactionWork(sleeveIndex, companyName, workPrio.find(workType => possibleWork.has(workType)));
						} catch (e) {
							continue;
						}
					}
					const freshTask = ns.sleeve.getTask(sleeveIndex);
					ns.printf("Sleeve %d: company-faction-work %s for %s [%s]", sleeveIndex, freshTask.factionWorkType, companyName, ns.formatNumber(companyFavor));
					return;
				}
				continue;
			}
		...
		for (let factionName of factions) {
			...
			if (ns.getPlayer().factions.includes(factionName) && factionFavor < favorGoal) {
  			if (task?.factionName !== factionName) {
					const workPrio = ['security', 'field', 'hacking'];
					const possibleWork = new Set(ns.singularity.getFactionWorkTypes(factionName));
					if (!possibleWork.size) continue;
					try {
		  			ns.sleeve.setToFactionWork(sleeveIndex, factionName, workPrio.find(workType => possibleWork.has(workType)));
					} catch (e) {
						continue;
					}
				}
				const freshTask = ns.sleeve.getTask(sleeveIndex);
 				ns.printf("Sleeve %d: faction-work %s for %s [%s]", sleeveIndex, freshTask.factionWorkType, factionName, ns.formatNumber(factionFavor));
				return;
			}
		}
