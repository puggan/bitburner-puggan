...
export async function main(ns) {
    ...
    while(true) {
        ...
        if (growRequired) {
            if (weakenRequired) {
                // Grow 50%
                ram += relayScript(growScript, (relayMaxRam - ram) / 2, Infinity);

                // Weaken 50%
                ram += relayScript(weakenScript, relayMaxRam - ram, Infinity);
                continue;
            }

            // Grow 100%            
            ram += relayScript(growScript, relayMaxRam - ram, Infinity);
            continue;
        }
        ...
        ns.tprint(ns.nFormat(maxThreads * moneyPartPerThread * ns.getServerMaxMoney(target), '0 a') + ' from hacking ' + target + ' from ' + relay)
    }
}