...
export async function main(ns) {
	...
	for (let i = 0; i < ramSizes.length; i++) {
		let ramSize = ns.nFormat(ramSizes[i] * (1 << 30), "0 ib");
		let serverCost = ns.getPurchasedServerCost(ramSizes[i]);
		let canAfford = Math.floor(money / serverCost);
		let totalCost = ns.nFormat(Math.min(canAfford, buyableServers) * serverCost, "$0a");
		...
		if (canAfford > buyableServers) {
			canAfford = buyableServers + "+";
		} else if (canAfford === 0) {
			canAfford = "-";
		}
