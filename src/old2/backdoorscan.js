...
export async function main(ns) {
	...
	for (const serverName of servers) {
		...
		if (server.backdoorInstalled) {
			...
			if (
				ns.args.length < 1
				|| ns.args.includes(serverName)
				|| ns.args.length == 1 && serverName.indexOf(ns.args[0]) >= 0
			) {
	...
	await ns.write("paths.json.txt", JSON.stringify(paths, null, 2));
	for(const serverName of myServers) {
		await ns.scp(["serverList.txt", "rootList.txt", "moneyServers.txt", "h2.js", "workAll.js"], serverName);
	}
