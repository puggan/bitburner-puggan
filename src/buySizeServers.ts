...
export async function main(ns: NS) {
    const spareMoney = (ns.args.length > 1 ? +ns.args[1] : 1) * 1024 * 1024 * 1024;

    const ramLimit = ((size: string) => {
        switch (size) {
            case '1P':
            case '1PB':
            case '1000':
    ...
    const upgradeServer = async (ramLimit: number) => {
    ...
    const buyNewServer = async (ram: number) => {
