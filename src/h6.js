 ...
 * @returns {number}
 **/
export function serverPrio(ns, serverName) {
 ...
 **/
export function serverMemoryList(ns) {
	...
	for(const serverName of serverNames) {
		...
		if (serverName === 'home') {
			...
			servers.push({serverName, freeRam: freeRamAllowed, usedRam, maxRam: maxRamAllowed});
		} else {
 ...
 **/
export async function step(ns, verbose) {
	ns.clearLog();
	...
	const wekaenNormalPower = ns.weakenAnalyze(1);
	const wekaenHomePower = ns.weakenAnalyze(1, homeCores);
			...
			targets.push(
				{
					serverName,
					action: 'weaken',
					threads: weakenRequired / wekaenNormalPower,
					prio,
				}
			);
			continue;
	...
	const actionPriority = { "weaken": 2, "hack": 1 }; 
	...
	for (const executingServerName of ['home', ...ns.getPurchasedServers(), ...moneyServers]) {
		for (const process of ns.ps(executingServerName)) {
			for (const argument of process.args) {
				if (!moneyServers.includes(argument)) {
					continue;
				}
	...
	for (const targetData of validTargets) {
		...
		if (currentActions[targetData.serverName]) {
			if (currentActions[targetData.serverName].action !== 'weaken') {
				if(verbose) ns.printf("%s on %s skipped, already running %s (%s)", targetData.action, targetData.serverName, currentActions[targetData.serverName].action, currentActions[targetData.serverName].time ? timeFormat(currentActions[targetData.serverName].time - timeStamp) : '-');
				continue;
			}
		...
		if (threadsNeeded < 1) {
			if (targetData.threads < 1) {
				...
				if(verbose) ns.printf("%s on %s skipped, enough threads already running (%s)", targetData.action, targetData.serverName, currentActions[targetData.serverName].time ? timeFormat(currentActions[targetData.serverName].time - timeStamp) : '-');
			}
			continue;
		}
		...
		for (const serverToUse of serversToUse) {
			...
			if (serverToUse.serverName === 'home') {
				...
				if (homeCores > 1 && targetData.action === 'grow') {
					...
					threadOnHomeServer = Math.min(Math.ceil(threadsNeeded * wekaenNormalPower / wekaenHomePower), threadsToUse);
				}
